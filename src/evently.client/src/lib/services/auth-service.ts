...
export async function login(redirectUrl: string) {
	...
	// cannot use goto()
	window.location.href = `/api/v1/Auths/external/google/login?originUrl=${redirectUrl}`;
}

export async function logout(redirectUrl: string): Promise<void> {
	await axios.post("/api/v1/Auths/external/logout", {}, { params: { redirectUrl } });
...
export async function getAccount(): Promise<Account | null> {
	try {
		const response = await axios.get<Account>("/api/v1/Auths/external/account");
